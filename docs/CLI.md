...
| `BPSA_REASONING_EFFORT` | No | - | Value of the `reasoning_effort` request field sent with every model call, e.g. `low`, `medium`, `high`, `xhigh` or `max` (the accepted set depends on the model; OpenAI and OpenRouter document `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Unset means the provider's default. High values spend most of `BPSA_MAX_TOKENS` on reasoning, so raise it together. Non-reasoning models may reject the field. `OpenAIServerModel` only. |
