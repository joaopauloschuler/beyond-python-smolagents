...
POSTPEND_CLAUDE = '--web_search true'
POSTPEND_STRING = POSTPEND_CLAUDE
...
what_to_code = """a task manager using  mormot2 
( https://github.com/synopse/mORMot2 ) with SQLite3, ORM, SOA and MVC. The client application will be web based."""

fileext='.pas'
folder_name='solution1'

mormot_file = os.path.dirname(os.path.abspath(__file__))+"/mormot.md"
print("Loading:"+mormot_file)
mormot_example = load_string_from_file(mormot_file)
print(mormot_file+" loaded with "+str(len(mormot_example))+" characters.")
...
task = """Using only the """+computing_language+""" computing language for the server side, code a """+what_to_code+" ."
...
<your-task>
Inside the """+folder_name+""" folder, pick the next best feature to code or documentation to update and do it.
